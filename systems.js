...
function lineBuilder(rawSales){
    ...
    while (!(isLastElement(currentLine))){
        ...
        if (description.indexOf("can be attached to") !== -1) {
            ...
            itemPrice = currentLine.slice(0, currentLine.indexOf("\t")); // $9.99
...
function calculateCommission(lineItem){
    ...
    if (isNotServicePlan(lineItem)) {
        commission = lineItem.price_individual * commissionRate;
    }
...
function processSales(){
    commissionRate = document.getElementsByName("commission-rate")[0].value;
